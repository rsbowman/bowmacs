...
#+end_src
*** Popup buffer sizes
Don't change popup buffer heights (like grep-mode buffers from embark export) when visiting links.

#+begin_src elisp
(after! popup
  (defadvice! bowmacs--preserve-popup-size-a (fn &rest args)
    "Prevent popup windows from being resized when following compilation links."
    :around #'compilation-goto-locus
    (let* ((popup-windows (cl-remove-if-not #'+popup-window-p (window-list)))
           (popup-heights (mapcar (lambda (w) (cons w (window-height w)))
                                  popup-windows)))
      (prog1 (apply fn args)
        (dolist (pair popup-heights)
          (let ((window (car pair))
                (height (cdr pair)))
...
This also works, but doesn't let you resize even manually:
...
#+end_src
*** corfu/cape/dabbrev completion
#+begin_src elisp
(setq dabbrev-case-fold-search nil)    ; Case-sensitive search
(setq dabbrev-case-replace nil)        ; Don't adjust case of expansions
(setq cape-dabbrev-buffer-function #'cape-text-buffers) ; use completions from other buffers
#+end_src
*** wgrep
...
#+end_src
*** Projectile
Turn off file cache, it's often out of date
#+begin_src elisp
(after! projectile
...
**** Formatting bug
Work around a bug related to apheleia somehow, when using `+format/region-or-buffer` a "File to save to:" prompt appears.  Instead, call `lsp-format-buffer` directly when possible.
#+begin_src emacs-lisp 
(defun bowmacs--lsp-format-dwim ()
  "Format buffer or region using LSP directly, bypassing apheleia."
  (interactive)
  (if (and (bound-and-true-p lsp-mode) (lsp-feature? "textDocument/formatting"))
...
(map! :after format
      ...
      "c f" #'bowmacs--lsp-format-dwim)

#+end_src
**** Project root for monorepos
Have lsp-mode suggest the subproject root instead of monorepo root.
#+begin_src elisp
(defun bowmacs--lsp-suggest-project-root (advised-fn)
  "Find subproject root by monorepo specific markers."
  (or
...
(after! lsp-mode
  (advice-add 'lsp--suggest-project-root :around #'bowmacs--lsp-suggest-project-root))
#+end_src
*** vterm
#+begin_src elisp
(map! "<f6>" #'+vterm/toggle
...
** Load local.el
Per-machine config, for work stuff, email address & username, etc.
#+begin_src elisp
